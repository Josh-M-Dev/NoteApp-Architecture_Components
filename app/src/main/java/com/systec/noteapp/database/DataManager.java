...
public class DataManager{
    ...
    public List<NoteDao.noteJoincourse> getData;
    ...
    public  boolean checkNotes(){
        ...
        return dm.mNotes != null && dm.mCourses != null;
    }



    public static void loadNotesFromDatabase(List<NoteInfo> notes){
        Log.d(TAG, "loadNotesFromDatabase: ");
        DataManager dm = getInstance();
        dm.mNotes.clear();
        for (NoteInfo note : notes){
            CourseInfo noteCourse = dm.getCourse(note.getCourseId());
            dm.mNotes.add(new NoteInfo(note.getId(),noteCourse,note.getCourseId(), note.getTitle(), note.getText()));
        }

    }

    public static void loadCoursesFromDatabase(List<CourseInfo> courses) {
        DataManager dm = getInstance();
        dm.mCourses.clear();
        for (CourseInfo course : courses){
            dm.mCourses.add(new CourseInfo(course.getCourseId(), course.getTitle()));
        }
    }
    ...
    public int createNewNote() {
        CourseInfo noteCourse = null;
        NoteInfo note = new NoteInfo(noteCourse, null, null);
        mNotes.add(note);
...
//    //region Initialization codemPrograms
//
